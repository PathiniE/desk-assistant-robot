...
String p6 = "Always be polite, approachable, and clear, while maintaining a professional tone. The name of the department head is Dr. Ravindra De Silva";
...
void setup() {
  ...
  // Initialize I2S once for both RX and TX
  auto config = i2s.defaultConfig(TX_MODE);
  config.sample_rate = 16000;
  config.channels = 2; // Speaker uses stereo
  ...
  config.buffer_count = 8; // Increased for smoother streaming
  config.buffer_size = 256; // Increased for lower latency
  i2s.begin(config);
...
void loop() {
  static enum { GREETING, WAIT, COUNTDOWN, RECORD, PROCESS, NEXT } convState = GREETING;
  ...
  case GREETING:
      digitalWrite(R_LED, LOW);
      digitalWrite(B_LED, HIGH);
      playTTS("Hello, I am Lexi, your AI assistant. How can I help you today?");
      convState = WAIT;
    break;  
  ...
  case PROCESS:
    ...
    if (userInput.length() > 0) {
      ...
      if (userInput.indexOf("Lexi it's me I am available now") >= 0) {
        ...
        playTTS("Welcome back, department head. I have prepared a summary of all interactions.");
        playTTS(report);
...
String recordAndSTT() {
  ...
  int totalSamples = 16000 * 5;  //seconds of recording
  int samplesRead = 0;
...
// Play TTS Audio (streamed directly to decoder)
void playTTS(String text) {
  WiFiClientSecure client;
  client.setInsecure();
  HTTPClient http;
  http.setTimeout(10000); // Reduced from default 30s to fail faster
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    uint8_t temp[2048]; // Increased buffer size
    unsigned long startTime = millis();
    const unsigned long timeout = 1500; // Reduced timeout to 1.5s

    while ((stream->connected() || stream->available()) && (millis() - startTime < timeout)) {
      int len = stream->available();
      if (len > 0) {
        int readLen = stream->readBytes(temp, std::min<size_t>(sizeof(temp), len));
        if (readLen > 0) {
          audioStream.write(temp, readLen); // Stream directly to decoder
          startTime = millis(); // Reset timeout on data
        }
      } else {
        delay(5); // Reduced delay to minimize latency
      }
    }

    audioStream.flush(); // Ensure remaining data is processed
    audioStream.end();
    DEBUG_PRINT("Finished streaming and playing MP3");
  } else {
